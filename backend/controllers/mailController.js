...
exports.sendMail = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "Mail sent successfully", mail });
  } catch (err) {
...
exports.getInbox = async (req, res) => {
  try {
    ...
    const mails = await Mail.find({ receiver: userId, receiverDeleted: false }).sort({ sentAt: -1 });
    const unreadCount = await Mail.countDocuments({ receiver: userId, isRead: false, receiverDeleted: false });
...
exports.getSentBox = async (req, res) => {
  try {
    ...
    const mails = await Mail.find({ sender: userId, senderDeleted: false }).sort({ sentAt: -1 });
...
exports.deleteMail = async (req, res) => {
  try {
    ...
    if (mail.sender.toString() === userId || mail.receiver.toString() === userId) {
      ...
      return res.status(200).json({ success: true, message: "Mail marked as deleted" });
    }
